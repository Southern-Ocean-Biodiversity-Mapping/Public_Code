...
modeltype = 1
model = 1
load(paste0("/pvol3TB/2_fitting_and_running_models/",res,"_model_cells_",model,"_pa_chains_4_thin_10_samples_800.Rdata"))
pa <- models[[2]]

## abundance model
modeltype = 2
model = 1
load(paste0("/pvol3TB/2_fitting_and_running_models/",res,"_model_cells_",model,"_abundance_chains_4_thin_10_samples_800.Rdata"))
ab <- models[[2]]
...
## setup prediction - pa
#GRADIENT IS DOING WEIRD STUFF TO THE PREDICTIONS: Gradient.pa = prepareGradient(pa, XDataNew = XData.grid.loop, sDataNew = list(cellID = xy.grid.loop))
predY.loop.pa <- predict(pa, XData=XData.grid, expected=TRUE) #predict(pa, Gradient=Gradient.pa, expected=TRUE) ## this gives probabilities instead of integer outcomes
#rm(Gradient.pa)

## setup prediction - abund
#Gradient.ab = prepareGradient(ab, XDataNew = XData.grid.loop, sDataNew = list(cellID = xy.grid.loop))
predY.loop.ab <- predict(ab, XData=XData.grid, expected=TRUE) #predict(ab, Gradient=Gradient.ab, expected=TRUE) ## this gives probabilities instead of integer outcomes
#rm(Gradient.ab)  

## Multiply matrices
predY.loop.pa.array = simplify2array(predY.loop.pa)
predY.loop.ab.array = simplify2array(predY.loop.ab)
## this calculates abundance (as opposed to abundance conditional on presence)
predY.loop = predY.loop.pa.array * predY.loop.ab.array
...
## save-string
dat.name <- paste0("/pvol3TB/3_model_analysis/",res,"_model_cells_",model,"_pa_and_abundcondpres_chains_4_thin_10_samples_800_PredVsObs_envonly.Rdata")
...
save(predY.mean, predY.median, predY.5, predY.95,
     ...
     predY.ab.mean, predY.ab.median, predY.ab.5, predY.ab.95, XData.grid.loop, xy.grid.loop,
     file=dat.name)
...
sel.list <- split(1:454,ceiling(seq_along(1:454)/15))

ptm = proc.time()
foreach(j=1:length(sel.list)) %dopar%{ 
  ## setup environmental data
  XData.grid.loop <- XData.grid[sel.list[[j]],]
  xy.grid.loop <- xy.grid[sel.list[[j]],]
  ...
  dat.name <- paste0("/pvol/biodiversity_prediction/pred_files_test/",res,"/abundance/",
...
# 
# ## save data
# save(sel, sel.not.na, file=paste0("/pvol/biodiversity_prediction/hmsc_",res,"_model_cell_sel.Rdata"))
# save(XData.grid, xy.grid, file=paste0("/pvol/biodiversity_prediction/hmsc_",res,"_model_cell_grid.Rdata"))
...
pred.list <- list.files("/pvol/biodiversity_prediction/pred_files/2km/abundance/")
pred.list <- pred.list[grep("800_pred",pred.list)]
pred.list <- pred.list[grep("abundance",pred.list)]
...
foreach(j=try.again.v) %dopar% {
  ...
  ## name for saving:
  dat.name <- paste0("/pvol/biodiversity_prediction/pred_files/",res,"/abundance/",res,"_model_cells_", as.character(model), "_",
  ...
  ## presence-absence part - prediction, mean, median and CIs
  predY.loop.pa.array = simplify2array(predY.loop.pa)
  predY.pa.mean = as.data.frame(apply(predY.loop.pa.array, 1:2, mean))
  predY.pa.median = as.data.frame(apply(predY.loop.pa.array, 1:2, median))
  predY.pa.5 = as.data.frame(apply(predY.loop.pa.array, 1:2, p5))
  ...
  ## presence-absence part - save
  save(predY.pa.mean, predY.pa.median, predY.pa.5, predY.pa.95,
       sel.loop, XData.grid.loop, xy.grid.loop,
       file=paste0(dat.name,"pa_only_",run.name,".Rdata"))
  rm(predY.pa.mean, predY.pa.median, predY.pa.5, predY.pa.95)
  
  # ## Multiply matrices
  # predY.loop = predY.loop.pa.array * simplify2array(predY.loop.ab)
  # rm(predY.loop.pa.array)
  #    
  # ## Get posterior median and CIs
  # predY.mean = as.data.frame(apply(predY.loop, 1:2, mean))
  # predY.median = as.data.frame(apply(predY.loop, 1:2, median))
  # predY.5 = as.data.frame(apply(predY.loop, 1:2, p5))
  # predY.95 = as.data.frame(apply(predY.loop, 1:2, p95))
  ...
  #    predY.mean, predY.median, predY.5, predY.95,
  #    sel.loop, XData.grid.loop, xy.grid.loop)
  rm(predY.loop.pa, sel.loop, XData.grid.loop, xy.grid.loop)
}
computational.time = proc.time() - ptm
parallel::stopCluster(cl = c1)
